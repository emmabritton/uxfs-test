...
pub fn render_ui(controller: &Controller, graphics: &mut Graphics) {
    ...
    draw_button(graphics, '3', 166, 202, LIGHT_GRAY, &controller.button_shape);
    ...
    draw_button(graphics, '9', 140, 260, LIGHT_GRAY, &controller.button_shape);
    draw_button(graphics, '0', 208, 260, LIGHT_GRAY, &controller.button_shape);
}

fn draw_item(graphics: &mut Graphics, item: &Item, value: &State, x: usize, y: usize, button_shape: &Shape) {
    ...
    graphics.draw(&Text::new(item.name, (x as isize + 50, y as isize), (WHITE, Large)));
    let text = match item.item_type {
        ItemType::Float => format!("{:0.1}", value.num()),
        ItemType::Int => format!("{}", value.num().round() as usize),
    };
    graphics.draw(&Text::new(text, (x as isize + 150, y as isize), (vcolor, Large)));
}

fn draw_button(graphics: &mut Graphics, letter: char, x: usize, y: usize, color: Color, shape: &Shape) {
...
pub fn generate_text() -> Vec<Text> {
    ...
    vec![
        ...
        Text::new("Shift to inc/dec faster", (100_isize,22), (general_text_color, Small)),
        Text::new("Oscillator", (4_isize, 186), (general_text_color, Large)),
        Text::new("Duty Cycle", (4_isize, 244), (general_text_color, Large)),
        Text::new("Space to play", (65_isize, 300), (general_text_color, Large)),
    ]
}
